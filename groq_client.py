...
def nlu_intencao(texto: str) -> dict:
    """Pede para a IA estruturar a intenção do usuário.
    Saída esperada:
    {
    ...
    if not GROQ_API_KEY:
        ...
        if "remover" in lower or "tirar" in lower:
            return {"acao":"remover","itens":[{"nome": lower.replace("remover","").replace("tirar","").strip(), "quantidade":1}]}
        if "quero" in lower or "add " in lower:
            ...
            try:
                ...
                pass
            nome = lower.replace("quero","").replace("add","").strip()
            return {"acao":"adicionar","itens":[{"nome": nome, "quantidade": qtd}]}
