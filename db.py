...
def buscar_produto_por_nome(nome: str):
    return fetch_one(
        "SELECT id, nome, preco_real FROM cardapio WHERE ativo = TRUE AND LOWER(nome) LIKE %s LIMIT 1",
        (f"%{nome.lower()}%",)
    )

def get_or_create_carrinho_aberto(cliente_id: int):
    carr = fetch_one(
        "SELECT * FROM carrinhos WHERE usuario_id=%s AND status='aberto' ORDER BY criado_em DESC LIMIT 1",
        (cliente_id,)
    )
    if carr:
        return carr
    return fetch_one(
        "INSERT INTO carrinhos (usuario_id, status) VALUES (%s,'aberto') RETURNING *",
        (cliente_id,)
    )
...
def listar_itens_carrinho(carrinho_id):
    return fetch_all(
        """SELECT ic.produto_id, c.nome, c.preco_real, ic.quantidade, 
                  (c.preco_real * ic.quantidade) AS subtotal
...
def total_carrinho_reais(carrinho_id) -> float:
    res = fetch_one(
        """SELECT COALESCE(SUM(c.preco_real * ic.quantidade),0) AS total
             FROM itens_carrinho ic
             JOIN cardapio c ON c.id = ic.produto_id
             WHERE ic.carrinho_id=%s""",
        (carrinho_id,)
...
def finalizar_carrinho_e_criar_pedido(cliente_id, carrinho_id):
    carr = fetch_one(
        "UPDATE carrinhos SET status='fechado', finalizado_em=now() WHERE id=%s RETURNING *",
        (carrinho_id,)
    )
    ped = fetch_one(
        "INSERT INTO pedidos (cliente_id, carrinho_id, status) VALUES (%s,%s,'aguardando_pagamento') RETURNING *",
...
def criar_pagamento_ficticio(carrinho_id, tipo: str):
    pg = fetch_one(
        "INSERT INTO pagamentos (carrinho_id, tipo_pagamento, status, dados_brutos) "
        "VALUES (%s,%s,'pendente','{\"mock\":true}') RETURNING *",
        (carrinho_id, tipo)
    )

    pg = fetch_one("UPDATE pagamentos SET status='concluido' WHERE id=%s RETURNING *", (pg["id"],))
    ped = fetch_one(
        "UPDATE pedidos SET status='concluido', atualizado_em=now() WHERE carrinho_id=%s RETURNING *",
        (carrinho_id,)
    )
    return pg, ped
